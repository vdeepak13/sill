...
int main(int argc, char* argv[]) {
  ...
  boost::shared_ptr<array_dataset> ds_ptr
    = oracle2dataset<array_dataset>(knorm, nrecords);
  array_dataset& ds = *ds_ptr;
  boost::shared_ptr<vector_dataset> vecds_ptr
    = oracle2dataset<vector_dataset>(knorm, nrecords);
  vector_dataset& vecds = *vecds_ptr;
  // Create some views
  dataset_view ds_view_range(vecds);
  ds_view_range.set_record_range(1000, 2000);
  std::vector<size_t> indices;
  for (size_t i = 1000; i < 2000; i++)
    indices.push_back(i);
  dataset_view ds_view_indices(vecds);
  ...
  for (size_t n = 0; n < nruns; n++) {
    array_dataset::record_iterator r_it = ds.records().first;
  ...
  cout << " array_dataset: " << t.elapsed() / nruns << std::endl;
  t.restart();
  for (size_t n = 0; n < nruns; n++) {
    vector_dataset::record_iterator r_it = vecds.records().first;
    for (size_t i = 0; i < 1000; i++) {
      const vec& v = (*r_it).vector();
      for (size_t j = 0; j < nvars; j++)
        tmp += v[j];
...
*/
  ...
  if (1) { // since I haven't uploaded the UCI datasets to SVN
    symbolic_oracle o =
      create_symbolic_oracle("/Users/jbradley/data/uci/adult/adult-test.sum",u);
  ...
  cout << "First 6 records from vecds and variable view with only "
       << "finite var 0 and vector vars 1-5:\n"
       << " Finite vals:\n";
  for (size_t i = 0; i < 6; i++) {
    cout << "Original: " << vecds[i].finite() << "\n"
         << "Var view: " << ds_view_vars[i].finite() << "\n";
  }
  cout << " Vector vals:\n";
  for (size_t i = 0; i < 6; i++) {
    cout << "Original: " << vecds[i].vector() << "\n"
