#ifndef MPI_ROUNDROBIN_BP
#define MPI_ROUNDROBIN_BP
...
namespace prl {
  ...
  private:
    ...
    mpi_adapter< mpi_round_robin_bp<F> > *adapter_;
  ...
  public:
    ...
    mpi_round_robin_bp(mpi_post_office &po): po_(po), csr_(sum_product) {
      universe_ = NULL;
      local_graph_ = NULL;
    ...
    void initialize_root(universe* universe,
      ...
      adapter_ = new mpi_adapter< mpi_round_robin_bp<F> >(po_, *consensus_);
      po_.barrier();
      // partition the graph
...
// use the consensus protocol
  ...
  }; // End of class mpi_round_robin_bp
