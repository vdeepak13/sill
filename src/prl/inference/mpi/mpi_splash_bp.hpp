...
namespace prl {
  ...
  public:
    ...
    void initialize_root(universe* universe,
      ...
      create_messages();
      std::cout << po_.id() << " initialized: "<<splash_size_<<" " << bound_<<"\n";
      std::cout.flush();
...
//        double delta = norm_1(messages_[srcid][targetid], new_msg);
        ...
        adapter_->queue_outgoing_bp_message(source, target, new_msg,0.0);
//        adapter_->send_outgoing_bp_message(source, target, new_msg);
...
//      }
    ...
    void splash(const vertex_type& root) {
      ...
      revforeach(size_t vid, splash_order) {
        if (schedule_.get(vid) >= bound_ / UPDATE_BOUND) {
      ...
      for (size_t i = 1; i < splash_order.size(); ++i) {
        if (schedule_.get(splash_order[i]) >= bound_ / UPDATE_BOUND) {
      ...
      */
      ...
      void generate_splash(const vertex_type& root,
        ...
        size_t work = local_graph_->work_per_update(root);
        //if (adapter_->on_boundary(root)) work = work / 2;
        work_in_queue += work;
        ...
        double limit = bound_ /10.0;
        double topres = schedule_.get(root.id());
        if (topres > 100) limit = 100;
        //else if (topres / 10.0 < bound_) limit = bound_ / 10.0;
        ...
        for(work_in_splash = 0; work_in_splash < splash_size_
          ...
          //std::cout << uid << ": " << local_graph_->work_per_update(uid) << "\n";

          splash_queue.pop_front();
          size_t work = local_graph_->work_per_update(uid);
          //if (adapter_->on_boundary(uid)) work = work / 2;
          ...
          if(work_in_queue + work_in_splash < splash_size_) {
            ...
            foreach(const vertex_type& v, local_graph_->neighbors(uid)) {
              if((adapter_->own_vertex(v)) && (visited.count(v.id()) == 0)
                && work_in_splash + local_graph_->work_per_update(v) < splash_size_
                && schedule_.get(v.id()) >= limit) {
                splash_queue.push_back(v.id());
                visited.insert(v.id());
                size_t work = local_graph_->work_per_update(v);
                //if (adapter_->on_boundary(uid)) work = work / 2;
                work_in_queue += work;
              }
      ...
      double loop_to_convergence() {
        ...
        size_t flushctr = 0;
        size_t bedge = 0;
        size_t aboveboundbedge = 0;
        while(1) {
          bool justwokeup = true;
          double postrecvcurtop ;
          double curtop;
          bool boundswitch = false;
          while(1) {
            // Receive any inbound messages
            adapter_->receive_bp_messages(this);
            size_t oldboundedge=boundaryedgeupdates_; 
            consensus_->begin_critical_section();
            curtop = schedule_.top().second; 
            if (justwokeup) postrecvcurtop = curtop;  
            if (nump == 1 || justwokeup) {
            ...
            if((ti.current_time() - lasttime) >= 4) {
              ...
              << " " << numsplashes_ << " "
              << double(numupdates_)/numsplashes_ << " " <<bound_<<std::endl;
            }
           
            // Send newly created outbound messages
            if (curtop >= initialbound) {
              ++flushctr;
            }
            else {
              boundswitch = true;
            }
            if (flushctr > 0 || (boundswitch==true && flushctr > 0)) {
                adapter_->flush(this, false);
                flushctr = 0;
            }
//            }
          }  // While great than bound 
          // if I just wokeup from sleep (the second condition is so that 
          // this is not called the first time I go to sleep
          if (!justwokeup && firstloop == false)  {
            double gain = postrecvcurtop / bound_;
            bound_ = std::max(initialbound / 1000, (bound_) / 2);
            std::cout << "Bound at " << curtop <<  ": Lowering bound to " << bound_ << "\n";
          }
          firstloop = false;
