#ifndef BINARY_RESIDUAL_WORK_SPLASH_BP_HPP
#define BINARY_RESIDUAL_WORK_SPLASH_BP_HPP
...
namespace sill {
  ...
  template<typename F>
  class binary_residual_work_splash_bp {
    ///////////////////////////////////////////////////////////////////////
    // typedefs
  public:
  ...
  public:
     ...
     */
    binary_residual_work_splash_bp(factor_graph_type* factor_graph,
                       size_t splash_size,
                       double bound,
                       double damping) :
    ...
    void generate_splash(const vertex_type& root, 
      ...
      for(size_t i = 0; i < splash_size_ && !splash_queue.empty();) {
        // Remove the first element
        vertex_type u = splash_queue.front();
        splash_queue.pop_front();
        size_t numn = factor_graph_->num_neighbors(u);
        if (i==0 || i +numn <= splash_size_) {
        ...
        foreach(const vertex_type& v, factor_graph_->neighbors(u)) {
          if((visited.count(v) == 0) && residuals_[v] > bound_ && i+factor_graph_->num_neighbors(v) <= splash_size_) {
  ...
  /**
    ...
    void generate_splash_priority(const vertex_type& root, 
      ...
      for(size_t i = 0; i < splash_size_ && !splash_queue.empty();) {
        ...
        foreach(const vertex_type& v, factor_graph_->neighbors(u)) {
          if((visited.count(v) == 0) && residuals_[v] > bound_ && i+factor_graph_->num_neighbors(v) <= splash_size_) {
            splash_queue.push(v,residuals_[v] > bound_);
            visited.insert(v);
          }
        } // end of for each neighbors
      } // End of foor loop
    ...
    inline void send_messages(const vertex_type& source) {
      if (factor_graph_->num_neighbors(source) >= 1000) {
        std::cout << "!";
      }
