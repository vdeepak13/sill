...
#ifndef SILL_CONSTANT_AND_LAZY_FACTOR_HPP
#define SILL_CONSTANT_AND_LAZY_FACTOR_HPP
...
(factor_collapse_expr<lazy_factor_t<internal_factor_t,
  ...
  lazy_factor_t<internal_factor_t, csr_tag_t, elim_strategy_t>
    collapsed(sill::collapse(expr.x_ptr, expr.retained, binary_op_tag_t()));
  // To get the constant value of this factor (which has no
  // arguments), convert it to a constant factor.
  constant_factor<range_t>
    constant(sill::collapse(collapsed.flatten(),
                           domain::empty_set,
                           binary_op_tag_t()));
  this->value = constant.get_value();
}

#endif // #ifndef SILL_CONSTANT_AND_LAZY_FACTOR_HPP
