...
namespace sill {
  ...
  void all_pairs_batch::build(dataset_statistics<la_type>& stats) {
    ...
    for (size_t i = 0; i < nclasses_ - 1; i++) {
      for (size_t j = i+1; j < nclasses_; j++) {
        ...
        dataset_view<la_type> ds_view(ds, true);
  ...
  /*
    ...
    dataset_view<la_type> ds_view_
    (ds, label_, params.binary_label, j);
    dataset_view<la_type> ds_view(ds_view_, ind);
    distrib.clear();
  ...
  */
  ...
  std::size_t all_pairs_batch::predict(const record_type& example) const {
