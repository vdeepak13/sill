...
#include <sill/learning/dataset3/slice_view.hpp>
...
namespace sill {
  ...
  public:
    ...
    vector_record2<T> record(size_t row) const {
      return record(row, args);
    }

    vector_record2<T> record(size_t row, const vector_var_vector& vars) const {
      assert(row < num_inserted);
      vector_record2<T> result(vector_size(vars), weights[row]);
      size_t col = 0;
