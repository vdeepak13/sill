...
namespace sill {
  ...
  template <typename LA> class vector_dataset_old;
  ...
  protected:
    ...
    using base::load_finite_record_old;
    using base::load_vector_record_old;
    ...
    dataset_view(vector_dataset_old<la_type>* ds_ptr, const dataset_view& ds_view_source)
  ...
  template <typename LA>
  void dataset_view<LA>::load_finite_record_old(size_t i, finite_record_old& r) const {
    if (m_new_var != NULL) {
      ...
      ds.load_finite_record_old(convert_index(i), r);
    }
  ...
  } // load_finite_record_old

  template <typename LA>
  void
  dataset_view<LA>::
  load_vector_record_old(size_t i, vector_record_old<la_type>& r) const {
    if (vv_view == ALL_VAR_INDICES || vv_view == VEC_VAR_INDICES) {
      ds.load_vector(convert_index(i), tmp_vecdata);
      r.vector() = tmp_vecdata(vv_vector_var_indices);
    } else {
      ds.load_vector_record_old(convert_index(i), r);
    }
  ...
  void dataset_view<LA>::convert_record(const record_type& orig_r,
    ...
    if (m_new_var != NULL) {
      ...
      if (vv_view == ALL_VAR_INDICES || vv_view == FIN_VAR_INDICES) {
        ...
        new_r.finite_record_old::operator=(orig_r);
      }
      if (vv_view == ALL_VAR_INDICES || vv_view == VEC_VAR_INDICES) {
        ...
        new_r.vector_record_old<la_type>::operator=(orig_r);
      }
    } else {
      new_r = orig_r;
    }
  ...
  dataset_view<LA>::create_light_view() const {
    vector_dataset_old<LA>* tmp_ds_ptr =
      new vector_dataset_old<LA>(ds.datasource_info());
