...
namespace sill {
  ...
  class vector_dataset_old : public dataset<LA> {
  ...
  public:
    ...
    explicit vector_dataset_old(const datasource_info_type& info);

    //! Constructs the datasource with the given sequence of variables.
    //! @param info    info from calling datasource_info()
    vector_dataset_old(const datasource_info_type& info,
                   size_t nreserved);
  ...
  protected:
    ...
    void load_finite_record_old(size_t i, finite_record_old& r) const {
      if (r.fin_own)
    ...
    void load_vector_record_old(size_t i, vector_record_old<la_type>& r) const {
      if (r.vec_own)
    ...
    // From datasource:
    using base::convert_finite_record_old2assignment;
    using base::convert_vector_record_old2assignment;
  ...
  };  // class vector_dataset_old

  //============================================================================
  // Implementations of methods in vector_dataset_old
  //============================================================================
  ...
  vector_dataset_old<LA>::vector_dataset_old() : base() { }

  template <typename LA>
  vector_dataset_old<LA>::vector_dataset_old
  (const finite_var_vector& finite_vars,
   const vector_var_vector& vector_vars,
   const std::vector<variable::variable_typenames>& var_type_order,
   size_t nreserved)
  ...
  template <typename LA>
  vector_dataset_old<LA>::vector_dataset_old(const datasource_info_type& info)
    : base(info) {
    reserve(1);
  }
  ...
  template <typename LA>
  void vector_dataset_old<LA>::save(oarchive& a) const {
    base::save(a);
    a << finite_data << vector_data;
  }

  template <typename LA>
  void vector_dataset_old<LA>::load(iarchive& a) {
    base::load(a);
    a >> finite_data >> vector_data;
  }
  ...
  template <typename LA>
  size_t vector_dataset_old<LA>::capacity() const {
    return finite_data.size();
  }

  template <typename LA>
  size_t vector_dataset_old<LA>::finite(size_t i, size_t j) const {
    assert(i < nrecords && j < num_finite());
    return finite_data[i][j];
  }

  template <typename LA>
  double vector_dataset_old<LA>::vector(size_t i, size_t j) const {
    assert(i < nrecords && j < dvector);
    return vector_data[i][j];
  }

  template <typename LA>
  typename vector_dataset_old<LA>::record_iterator_type
  vector_dataset_old<LA>::begin() const {
    if (nrecords > 0)
  ...
  template <typename LA>
  void vector_dataset_old<LA>::reserve(size_t n) {
    if (n > capacity()) {
...
//        vector_data[i].resize(dvector);
  ...
  template <typename LA>
  void vector_dataset_old<LA>::set_record(size_t i, const assignment& a, double w) {
    assert(i < nrecords);
    ...
    if (weighted) {
      ...
      std::cerr << "vector_dataset_old::set_record() called with weight w != 1"
  ...
  template <typename LA>
  void
  vector_dataset_old<LA>::set_record(size_t i, const std::vector<size_t>& fvals,
  ...
  template <typename LA>
  std::pair<vec, vec> vector_dataset_old<LA>::normalize() {
  ...
  template <typename LA>
  void vector_dataset_old<LA>::normalize(const vec& means,
                                     const vec& std_devs) {
  ...
  template <typename LA>
  void vector_dataset_old<LA>::normalize(const vec& means, const vec& std_devs,
                                     const vector_var_vector& vars) {
  ...
  template <typename LA>
  void vector_dataset_old<LA>::normalize2(const vector_var_vector& vars) {
    foreach(vector_variable* v, vars)
      assert(this->has_variable(v));
    uvec vars_inds(vector_indices(vars));
    for (size_t i(0); i < nrecords; ++i) {
  ...
  template <typename LA>
  void vector_dataset_old<LA>::randomize(double random_seed) {
...
//      vector_data[i].swap(vector_data[j]);
  ...
  template <typename LA>
  void
  vector_dataset_old<LA>::load_assignment(size_t i, sill::assignment& a) const {
    assert(i < nrecords);
    convert_finite_record_old2assignment(finite_data[i], a.finite());
    convert_vector_record_old2assignment(vector_data[i], a.vector());
  }

  template <typename LA>
  void
  vector_dataset_old<LA>::load_finite(size_t i, std::vector<size_t>& findata) const{
    findata = finite_data[i];
  }

  template <typename LA>
  void vector_dataset_old<LA>::load_vector(size_t i, vector_type& vecdata) const {
    vecdata = vector_data[i];
  }

  template <typename LA>
  void
  vector_dataset_old<LA>::load_assignment_pointer(size_t i, assignment** a) const {
    assert(false);
  }
