...
namespace sill {
  ...
  /**
   * Iterator for values in a finite_record_old.
  ...
  public:
     ...
     */
    finite_record_old_iterator
    (const finite_record_old& r, finite_variable* v);

    //! Prefix increment.
    finite_record_old_iterator& operator++();

    //! Postfix increment.
    finite_record_old_iterator operator++(int);
    ...
    bool operator==(const finite_record_old_iterator& it) const;
