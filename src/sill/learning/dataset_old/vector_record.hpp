#ifndef SILL_VECTOR_RECORD_OLD_HPP
#define SILL_VECTOR_RECORD_OLD_HPP
...
namespace sill {
   ...
   *
   * @todo Allow vector_record_olds to be restricted to a set of variables.
...
// TODO: FINISH RECORD VIEWS
    ...
    */
    ...
    //! Constructs an empty vector_record_old which owns its data.
    vector_record_old()
      : vector_numbering_ptr(new std::map<vector_variable*, size_t>()),
        vec_own(true), vec_ptr(new vector_type()) {
    }

    //! Constructor for a vector_record_old which owns its data.
    ...
    //! Constructor for a vector_record_old which uses data from its creator
    vector_record_old
    (copy_ptr<std::map<vector_variable*, size_t> > vector_numbering_ptr,
     vector_type* vec_ptr)
    ...
    //! Constructor for a vector_record_old which owns its data.
    explicit vector_record_old(const vector_var_vector& vector_seq,
                           size_t vector_dim)
      : vector_numbering_ptr(new std::map<vector_variable*,size_t>()),
    ...
    vector_record_old(const vector_record_old& rec)
    ...
    bool operator==(const vector_record_old& other) const {
    ...
    */
    ...
    vector_record_old& operator=(const vector_record_old& rec) {
    ...
    //! The new copy owns its data since a record does not know whether
    //! or not it's OK to rely on the outside reference.
    vector_record_old& operator=(const sill::vector_assignment& a) {
     ...
     */
    void copy_from_record_mapped(const vector_record_old& r,
                                 const vector_var_map& vmap) {
      for (std::map<vector_variable*, size_t>::const_iterator it =
             vector_numbering_ptr->begin();
           it != vector_numbering_ptr->end();
  ...
  std::ostream& operator<<(std::ostream& out, const vector_record_old<LA>& r) {
    r.write(out);
    return out;
  }
  ...
  template <typename VecType>
  void
  vector_record_old<sparse_linear_algebra<T,SizeType> >::
