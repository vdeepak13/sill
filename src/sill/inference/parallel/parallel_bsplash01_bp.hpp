#ifndef PARALLEL_BSPLASH01_BP_HPP
#define PARALLEL_BSPLASH01_BP_HPP
...
namespace sill {
  ...
  class parallel_bsplash01_bp:
    ...
    public:
      ...
      */
      class blfsplash_thread : public thread {
        public:
          ...
          size_t numedgeupdates_;
          size_t numops_;
          blfsplash_thread(parallel_bsplash01_bp &owner, size_t scheduleid)
      ...
      */
      parallel_bsplash01_bp():
          csr_(sum_product) {
        factor_graph_ = NULL;
        splash_size_ = 100;
        bound_ = 0.001;
        damping_ = 0.0;
      } // end parallel_bsplash01_bp
      ...
      void splash(const vertex_type& root, 
        ...
        foreach(const size_t vid,
          ...
          if (residuals_[vid] >= bound_ / 10) {
            send_messages(factor_graph_->id2vertex(vid));
            numops += factor_graph_->work_per_update(vid);
            numedg += factor_graph_->num_neighbors(vid);
            numupdates++;
          }
      ...
      */
      void generate_splash(const vertex_type& root,
        ...
        work_in_queue += factor_graph_->work_per_update(root);
        visited.insert(rootid);

        // Grow a breath first search tree around the root
        for(work_in_splash = 0; work_in_splash < splashsize
              && !splash_queue.empty();) {
          ...
          if(work_in_queue + work_in_splash < splashsize) {
            ...
            foreach(size_t vid, factor_graph_->neighbor_ids(uid)) {
              if(residuals_[vid] > bound_/10 && (visited.count(vid) == 0)) {
      ...
      inline double update_message(const vertex_type& source,
       ...
       // Update the message
        ...
        // if we are now above bound, the set the priority to 1
        if (residuals_[target.id()]<= bound_ && new_priority > bound_) {
          schedule_.update(target.id(), 1.0);
        }
      ...
      inline void send_messages(const vertex_type& source) {
        ...
        if (vertex_being_updating_[factor_graph_->vertex2id(source)].try_lock() == false) return;
      ...
      inline double send_message(const vertex_type& source,
        ...
        double belief_residual =  update_message(source, target, new_msg);
  ...
  }; // End of class parallel_bsplash01_bp
}; // end of namespace
