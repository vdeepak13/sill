#ifndef PARALLEL_ROUNDROBIN_BP_HPP
#define PARALLEL_ROUNDROBIN_BP_HPP
...
namespace sill {
  ...
  class parallel_roundrobin_bp:
    ...
    public:
      ...
      */
      class roundrobin_thread : public thread {
        public:
          /// a reference to the parent class
          parallel_roundrobin_bp &owner_;
          ...
          circular_queue<size_t> *queue_;
          
          roundrobin_thread(parallel_roundrobin_bp &owner, size_t scheduleid,
                            circular_queue<size_t> *cq)
          ...
          void run() {
            ...
            while(owner_.converged(scheduleid_) == false) {
              size_t vid = queue_->next();
         //     if (owner_.schedule_.get(vid) > owner_.bound_) {
                ...
                if (numupdates_ % 10000 == 0) {
                  if (owner_.timeout_ > 0 && ti.current_time() >= owner_.timeout_) break;
                  if(scheduleid_ == 0) {
                    std::cout << scheduleid_ << ": " << owner_.schedule_.top().second << std::endl;
      ...
      */
      ...
      double loop_to_convergence() {
        ...
        std::vector<roundrobin_thread*> threads;
        ...
        circular_queue<size_t> queue(schedule);
        ...
        for (size_t i = 0;i < numprocs_; ++i) {
          threads[i] = new roundrobin_thread(*this,i,&queue);
          threads[i]->start();
        }
  ...
  }; // End of class parallel_roundrobin_bp
