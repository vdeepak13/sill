#ifndef SILL_ALTERNATING_FACTOR_FUNCTOR_HPP
#define SILL_ALTERNATING_FACTOR_FUNCTOR_HPP
...
namespace sill {
  ...
  /**
   * Wrapper for other random_factor_functor_i types which permits variety
   * via alternation between multiple random_factor_functor_i instances.
   *
   * This takes two random_factor_functor_i instances.
  ...
  struct alternating_factor_functor
    ...
    //! Constructor.
    explicit
    alternating_factor_functor(unsigned random_seed = time(NULL))
      : cnt(0) {
      seed(random_seed);
    }
...
#endif // SILL_ALTERNATING_FACTOR_FUNCTOR_HPP
