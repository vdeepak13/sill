...
BOOST_FIXTURE_TEST_CASE(test_records, fixture) {
  ...
  // matches the factor for every variable or pairs of variables
  table_factor_learner<> learner(ds);
  for (size_t i = 0; i < v.size(); ++i) {
    for (size_t j = i; j < v.size(); ++j) {
      finite_domain dom = make_domain(v[i], v[j]);
      table_factor mle = learner(dom);
...
BOOST_FIXTURE_TEST_CASE(test_subset, fixture) {
  ...
  table_factor_learner<> learner1(ds1);
  table_factor mle1 = learner1(make_domain(v));
  ...
  table_factor_learner<> learner2(ds2);
  table_factor mle2 = learner2(make_domain(v[2]));
...
BOOST_FIXTURE_TEST_CASE(test_shuffle, fixture) {
  table_factor_learner<> learner(ds);
  table_factor mle1 = learner(make_domain(v));
  ds.shuffle(rng);
  table_factor mle2 = learner(make_domain(v));
